...
class SQLExport:
    class BaseSQLExport():
        ...
        def initialize_tables( self):
            self.tables = {}
            self.tables['Movie'] = SQLTable( self.unprocessed_film_data)
    ...
    class TaglineSQLExport( BaseSQLExport):
        def initialize_tables( self):
            ...
            self.tables['Tagline'] = SQLTable( self.unprocessed_tagline_data)
            ...
            for film in self.films:
                if FilmRecord.TAGLINES_FLAG in film.metadata_flags:
                    dat = dat + [{
                        ...
                        'tagline_text': SQLTable.text_field_m( tagline)}
    ...
    class ProductionSQLExport( BaseSQLExport):
        def initialize_tables( self):
            super().initialize_tables()
            self.tables['MovieProduction'] = SQLTable( self.unprocessed_production_data)
        ...
        def unprocessed_production_data( self):
            ...
            for film in self.films:
                if FilmRecord.PROD_COS_FLAG in film.metadata_flags:
                    dat = dat + [{
                        'film_id'   : film.film_id,
                        'production_name': SQLTable.text_field_s( prod_co)}
    ...
    class GenreSQLExport( BaseSQLExport):
        def initialize_tables( self):
            super().initialize_tables()
            self.tables['MovieGenre'] = SQLTable( self.unprocessed_genre_data)
        ...
        def unprocessed_genre_data( self):
            ...
            for film in self.films:
                if FilmRecord.DETAILED_FLAG in film.metadata_flags:
                    dat = dat + [{
                        'film_id'   : film.film_id,
                        'genre_name': SQLTable.text_field_s( genre)}
    ...
    class SmallCastSQLExport( BaseSQLExport):
        def initialize_tables( self):
            super().initialize_tables()
            self.tables['RoleCode'] = SQLTable( [ {'role_name' : role } for role in ['Director', 'Writer', 'Actor'] ])
            self.tables['RoleCode'].AddPrimaryKey('role_code')
            self.tables['Role'] = SQLTable( self.unprocessed_small_cast_data)
            ...
            for film in self.films:
                if FilmRecord.DETAILED_FLAG in film.metadata_flags:
                    for person in film.metadata['directors']:
                        dat.append( {'film_id': film.film_id, 'person_name': SQLTable.text_field_s(person), 'role_code':0})
                    for person in film.metadata['writers']:
                        dat.append( {'film_id': film.film_id, 'person_name': SQLTable.text_field_s(person), 'role_code':1})
                    for person in film.metadata['actors']:
                        dat.append({'film_id': film.film_id, 'person_name': SQLTable.text_field_s(person), 'role_code':2})
            return dat
