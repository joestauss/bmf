...
class TestFilmWebscraper( unittest.TestCase):
    def setUp( self):
        self.known_metadata = {
            'tt0247745' : {
    ...
    def test_Film_TitleAndYear( self):
        for film_id, known_dd in self.known_metadata.items():
            if 'title' in known_dd and 'year' in known_dd:
                search_dd = Webscraper.IMDB_Film.title_and_year( film_id)
    ...
    def test_Film_Genres( self):
        for film_id, known_dd in self.known_metadata.items():
            if 'genres' in known_dd:
                search_dd = Webscraper.IMDB_Film.main_page( film_id)
    ...
    def test_Film_Taglines( self):
        for film_id, known_dd in self.known_metadata.items():
            if 'taglines' in known_dd:
                search_result = Webscraper.IMDB_Film.taglines( film_id)
    ...
    def test_Film_ProdCos( self):
        for film_id, known_dd in self.known_metadata.items():
            if 'prod co' in known_dd:
                self.assertTrue( known_dd['prod co'] in Webscraper.IMDB_Film.production_companies( film_id))
