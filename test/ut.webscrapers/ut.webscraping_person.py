...
class TestPersonWebscraper( unittest.TestCase):
    def setUp( self):
    ...
    def test_Person_FullName( self):
        self.assertEqual( 'Bruce Willis', Webscraper.IMDB_Person.full_name( 'nm0000246'))

    def test_Person_ActingFilmography( self):
        with self.assertRaises( ValueError):
            Webscraper.IMDB_Person.acting_filmography('I am not a valid person ID')

        for person_id, known_film_ids in self.known_filmography_entries:
            filmography = Webscraper.IMDB_Person.acting_filmography( person_id)
            for film in known_film_ids:
                self.assertTrue( film in filmography)

        filmography = Webscraper.IMDB_Person.acting_filmography(  'nm0000250' ) # renee zelwegger
