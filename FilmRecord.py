...
class BaseFilmRecord():
    def __init__(self, film_id):
        self.film_id, self.title, self.year = StringLocator.film_identity( film_id)
        self.details = []
    ...
    def identify( self):
        self.title, self.year = Webscraper.IMDB.Film.title_and_year( self.film_id)
...
class TaglineFilmRecord( BaseFilmRecord):
    def scrape_data(self):
        super().scrape_data()
        self.taglines = Webscraper.IMDB.Film.two_taglines_at_random( self.film_id)
        self.details.append( f'{len(self.taglines)} taglines')

class ProductionFilmRecord( BaseFilmRecord):
    def scrape_data(self):
        super().scrape_data()
        self.production_cos = Webscraper.IMDB.Film.production_companies( self.film_id)
...
class DetailedFilmRecord(TaglineFilmRecord, ProductionFilmRecord):
    def scrape_data(self):
        super().scrape_data()
        ttl_yr, sm_cst, detail, genres = Webscraper.IMDB.Film.main_page( self.film_id)
